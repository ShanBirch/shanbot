#!/usr/bin/env python3
"""
Get everyone who received Calendly links in the last 30 days from both tables.
"""
...
def get_recent_calendly_recipients():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()

    print("📅 EVERYONE WHO RECEIVED CALENDLY LINKS IN LAST 30 DAYS")
    print("=" * 70)
    ...
    recent_recipients = []
    for username, text, timestamp, source in all_recipients:
        try:
            if isinstance(timestamp, str):
                ...
                if dt >= start_date:
                    recent_recipients.append(
                        (username, text, timestamp, source))
        except:
            # If we can't parse the date, include it
            recent_recipients.append((username, text, timestamp, source))

    print(f"📅 TOTAL RECIPIENTS IN LAST 30 DAYS: {len(recent_recipients)}")
    print("-" * 50)

    for i, (username, text, timestamp, source) in enumerate(recent_recipients, 1):
    ...
    recent_usernames = [username for username, _, _, _ in recent_recipients]
    non_signups = [
        username for username in recent_usernames if username not in all_signups]

    for username in sorted(non_signups):
        print(f"  • @{username}")

    print(f"\n📊 SUMMARY:")
    print(f"  Total recipients: {len(recent_recipients)}")
    print(
        f"  Signed up: {len([u for u in recent_usernames if u in all_signups])}")
...
if __name__ == "__main__":
    get_recent_calendly_recipients()
