...
def generate_follow_up_message(conversation_history: List[Dict[str, Any]], topic: str) -> str:
    """Generate a follow-up message using Gemini with fallback models."""
    models = [
        'gemini-2.0-flash',
    ...
    {''.join([('User' if msg.get('type') == 'user' else 'Shannon') + ': ' + msg.get('text', '') + '\n' for msg in conversation_history])}
...
def get_user_bio(user_data: Dict[str, Any]) -> str:
    ...
    client_analysis = user_data.get('metrics', {}).get('client_analysis', {})
    bio = client_analysis.get('profile_bio', {})
    name = bio.get('PERSON NAME', '')
    interests = ', '.join(bio.get('INTERESTS', [])
                          ) if bio.get('INTERESTS') else ''
    summary = f"Name: {name}\nInterests: {interests}" if name or interests else "No bio available."
    return summary
...
def bulk_generate_followups(analytics_data: Dict[str, Any], followup_data: Dict[str, Any]):
    ...
    for responder_type in ['high_responders', 'medium_responders', 'low_responders']:
        for user in followup_data[responder_type]:
            ...
            message = generate_follow_up_message(conversation_history, topic)
...
def get_user_sheet_details(instagram_username: str) -> Dict[str, Any]:
    """
    Get user details from Google Sheet based on Instagram username.
    ...
    try:
        ...
        # Create a mapping of desired fields
        field_mapping = {
            'Email': 'email',
            'Phone number': 'phone',
            ...
            'weight': 'weight',
            'height': 'height',
            'long term fitness goals': 'fitness_goals',
            'Dietary Requirements': 'dietary_requirements',
            'Gym Acess': 'gym_access',
            'Training Frequency': 'training_frequency',
            'Exercises that dont fit with you?': 'excluded_exercises',
            'Exercises you enjoy': 'preferred_exercises',
            'daily cals': 'daily_calories',
            'preffered macro split': 'macro_split',
            'exclude foods': 'excluded_foods',
        ...
        for i, header in enumerate(headers):
            if header in field_mapping:
                col_indices[field_mapping[header]] = i
        ...
        for row in values[1:]:
            if instagram_col < len(row):
                sheet_username = row[instagram_col].strip().lower()
                if normalized_username == sheet_username:
                    # Found the user, extract all available information
                    user_details = {}
                    for field, index in col_indices.items():
                        if index < len(row) and row[index].strip():
                            user_details[field] = row[index].strip()
                    return user_details

        return {}

    except Exception as e:
        logger.error(f"Error getting user sheet details: {e}")
        return {}
