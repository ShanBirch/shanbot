...
def get_users_in_stage(analytics_data: Dict[str, Any], stage: str) -> int:
    ...
    count = 0
    for user_data in analytics_data.get('conversations', {}).values():
        metrics = user_data.get('metrics', {})
        if stage == "Topic 5" and metrics.get('trial_offer_made'):
            count += 1
        elif stage == "Topic 4" and metrics.get('topic4_completed'):
            count += 1
        elif stage == "Topic 3" and metrics.get('topic3_completed'):
            count += 1
        elif stage == "Topic 2" and metrics.get('topic2_completed'):
            count += 1
        elif stage == "Topic 1" and not any([
