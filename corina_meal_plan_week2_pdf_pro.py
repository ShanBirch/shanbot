#!/usr/bin/env python3
"""
Corina's Week 2 Meal Plan (PRO Layout)
- Uses cover with logo, centered title, client/date/goal, daily targets
- Renders Day 1-3 on separate pages in fixed order
- Shopping list on its own page; clickable links (Woolworths fallback)
- Removes any inline calculation lines
"""
...
# Pull the original meal text
from corina_meal_plan_week2_pdf import get_meal_plan_text as corina_text
...
def create_pdf():
    ...
    filename = os.path.join(
        OUTPUT_DIR, f"Corina_Week2_Meal_Plan_{current_date}_PRO.pdf")
    ...
    doc.build(story)
    print(f"✅ Corina Week 2 PRO PDF created: {filename}")
