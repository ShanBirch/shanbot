...
from fastapi.staticfiles import StaticFiles  # Corrected import for StaticFiles
...
from fastapi import FastAPI, Request, HTTPException, Header, Depends, BackgroundTasks, StaticFiles
...
# Serve the 'voice_samples' directory at the '/audio' path
app.mount("/audio", StaticFiles(directory="C:/Users/Shannon/OneDrive/Desktop/shanbot/voice_samples"), name="audio")
            ...
            # Handle Form Check Request (REVISED LOGIC - Priority pending check is now above)
                ...
                # And is_form_check_req is true (from main elif condition)
                    ...
                    if not form_check_pending.get(ig_username):
                        ...
                        response_time_value = "response time is action"
                ...
                if extracted_image_url:
                    ...
                    if not raw_gemini_analysis_result or "Error" in raw_gemini_analysis_result or "failed" in raw_gemini_analysis_result.lower():
                        ...
                        if update_manychat_fields(subscriber_id, field_updates):
                            ...
                            response_time_value_success = "response time is action"
    ...
    try:
        ...
        if not action_handled:
            ...
            if ai_raw_response_text:
                ...
                current_bot_reaction_time_s = time.time() - batch_start_time_s
                response_time_value_general = get_response_time_bucket(
                    current_bot_reaction_time_s)
                logger.info(
                    f"[GENERAL RESPONSE] Calculated reaction time: {current_bot_reaction_time_s:.2f}s, Bucket: {response_time_value_general} for SID {subscriber_id}")
...
async def _handle_buffered_messages_for_subscriber(subscriber_id: str, ig_username: str, message_payloads: List[Dict], bg_tasks: BackgroundTasks, batch_start_time_s: float):
    ...
    # --- New: Pre-process for detect_and_handle_action to include transcriptions --- START ---
    text_for_action_detection = final_combined_message_text
    # Check if the message is primarily a media URL, if so, try to get basic analysis/transcription
    # This is a simplified version of process_conversation_for_media, focused on getting text for intent detection
    url_pattern_for_action = r"^(https?://lookaside\.fbsbx\.com/ig_messaging_cdn/\?asset_id=[\w-]+&signature=[\w\-_.~]+)$\s*"
    match_for_action = re.fullmatch(
        url_pattern_for_action, final_combined_message_text)

    if match_for_action:
        media_url_for_action = match_for_action.group(1)
        logger.info(
            f"[_handle_buffered_messages_for_subscriber] Message is primarily a media URL: {media_url_for_action[:60]}. Attempting pre-action analysis.")
        # Use analyze_media_url which now handles transcription and basic image/video description
        media_type_action, analysis_text_action = analyze_media_url(
            media_url_for_action)
        if analysis_text_action:
            # Replace the URL with its analysis for action detection
            text_for_action_detection = analysis_text_action
            logger.info(
                f"[_handle_buffered_messages_for_subscriber] Using analyzed text for action detection: {text_for_action_detection[:150]}...")
        else:
            logger.warning(
                f"[_handle_buffered_messages_for_subscriber] Pre-action media analysis failed for URL. Proceeding with URL itself for action detection.")
    # --- New: Pre-process for detect_and_handle_action --- END ---
    ...
    if final_combined_message_text:
        new_combined_entry = {
            "type": "user",
            "text": final_combined_message_text,
            "timestamp": get_melbourne_time_str()
        }
        full_conversation_history.append(new_combined_entry)

    action_handled = await detect_and_handle_action(ig_username, text_for_action_detection, subscriber_id, last_full_payload, batch_start_time_s)
    ...
    if not action_handled:
        ...
        processed_user_message_for_prompt = process_conversation_for_media(
            final_combined_message_text
        )

        prompt_str_for_ai = build_member_chat_prompt(
            client_data=metrics_dict,
            current_message=processed_user_message_for_prompt,
            current_stage=current_stage,
            trial_status=trial_status,
            full_name=calculated_full_name
        )
...
async def _analyze_form_check_video(ig_username: str, client_name_for_todo: str, video_url: str, user_desc_text: str, subscriber_id: str, batch_start_time_s: float):
    ...
    time_diff_s = time.time() - batch_start_time_s
    response_bucket = get_response_time_bucket(time_diff_s)
    field_updates["response time"] = response_bucket
    logger.info(
        f"[_analyze_form_check_video] Calculated response time: {time_diff_s:.2f}s, Bucket: {response_bucket} for SID {subscriber_id}")
