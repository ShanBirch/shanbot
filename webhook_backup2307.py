...
async def handle_ad_response(ig_username: str, message_text: str, subscriber_id: str, first_name: str, last_name: str, user_message_timestamp_iso: str) -> bool:
    try:
        ...
        try:
            ...
            if general_auto_active:
                logger.info(
                    f"🤖 GENERAL AUTO MODE ACTIVE - Auto-processing ad response for {ig_username}")
                should_auto_process = True
            elif vegan_auto_active:
                # Check if user is a fresh vegan
                try:
                    ...
                    if result:
                        logger.info(
                            f"🌱 VEGAN AUTO MODE ACTIVE - Auto-processing fresh vegan ad response for {ig_username}")
                        should_auto_process = True
                    else:
                        logger.info(
                            f"🌱 Vegan auto mode active but {ig_username} is not a fresh vegan - manual review required for ad response")
                        should_auto_process = False
                except Exception as e:
                    logger.warning(
                        f"Could not check vegan status for {ig_username}: {e}")
                ...
                logger.info(
                    f"No auto mode active - {ig_username} ad response requires manual review")
                should_auto_process = False

        except ImportError:
            logger.warning(
                "Could not import auto_mode_state, assuming Auto Mode is OFF for ad response.")
            should_auto_process = False
