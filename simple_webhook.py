...
def get_or_create_user(ig_username: str, subscriber_id: str, first_name: str = "", last_name: str = ""):
    ...
    try:
        ...
        cursor.execute("SELECT * FROM users WHERE subscriber_id = %s", (subscriber_id,))
...
async def manychat_webhook(request: Request):
    ...
    try:
        ...
        # Get or create user
        user = get_or_create_user(ig_username, subscriber_id, first_name, last_name)
        ...
        # Add to review queue
        add_to_review_queue(ig_username, subscriber_id, message_text, ai_response)
