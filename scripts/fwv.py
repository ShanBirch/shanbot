...
class FitnessWrappedGenerator:
    def __init__(self, client_data, base_dir="."):
        ...
        # Set background video path
        self.background_video_path = self.base_dir / "blue2.mp4"
    ...
    def create_text_clip(self, text_elements, size=(1080, 1920), duration=None):
        """Create a video clip with text overlays"""
        duration = duration or self.slide_duration

        # Create a transparent background clip instead of black
        color_clip = ColorClip(size, color=(0, 0, 0, 0))
        ...
        for element in text_elements:
            ...
            # Make text color brighter
            color = element.get("color", '#ffffff')  # Pure white
            ...
            txt_clip = TextClip(
                ...
                stroke_width=2  # Stroke width for better visibility
            )
    ...
    def create_slide(self, text_elements=None, duration=None):
        ...
        try:
            ...
            # Create the text overlay clip
            if text_elements:
                text_clip = self.create_text_clip(
                    text_elements, duration=slide_duration)
            else:
                text_clip = ColorClip((1080, 1920), color=(
                    0, 0, 0, 0)).set_duration(slide_duration)

            # Create the slide with background video
            if self.background_video is not None:
                background_clip = self.background_video.set_duration(
                    slide_duration)
                # Use higher opacity for text (0.9 instead of 0.8)
                clip = CompositeVideoClip(
                    [background_clip, text_clip.set_opacity(0.9)])
                print(
                    f"Created slide with background video, duration: {slide_duration}s")
            else:
                clip = text_clip
                print("Created slide without background video")
    ...
    def build_video(self):
        ...
        if "conclusion" in self.available_templates:
            ...
            if conclusion_slide:
                final_conclusion_slide = self.create_slide(
                    conclusion_text, duration=3.0)
        ...
        if "motivational" in self.available_templates:
            ...
            motivational_slide = self.create_slide(
                motivational_text, duration=4.0)
