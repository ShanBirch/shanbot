...
import google_drive_utils  # Import the Google Drive utilities
...
class FitnessWrappedGenerator:
    ...
    def generate_fitness_wrapped_video(self, fitness_wrapped_data):
        ...
        try:
            ...
            if video_result:
                ...
                if drive_link:
                    return True, generator.output_path, drive_link
                else:
                    return True, generator.output_path, None
            else:
                logging.error(
    ...
    def create_slide(self, template_key, text_elements=None, duration=None):
        ...
        if text_elements:
            try:
                temp_image_path = self.add_text_to_image(
                    image_path, text_elements)
                # ImageClip expects a string path, not a Path object
                image_path_str = str(temp_image_path)
                clip = ImageClip(image_path_str).set_duration(slide_duration)
                ...
                if temp_image_path != image_path:
                    try:
                        ...
                        print(f"Could not remove temp file {temp_image_path}")
            except Exception as e:
                print(f"Error creating slide with text: {e}")
                # Fallback to using the template without text
    ...
    def build_video(self):
        ...
        print(f"Found {len(self.available_templates)} template slides to use.")
        slides = []

        # For vertical/story format, adjust vertical positioning
        center_y = 1000
        top_y = 500
        bottom_y = 1200
        line_height = 120
        ...
        if "workout_count" in self.available_templates and has_workout_data:
            workout_count_text = [
                {
                    "text": "You Trained",
            ...
            workout_count_slide = self.create_slide(
                "workout_count", workout_count_text)
            if workout_count_slide:
                slides.append(workout_count_slide)

        # 4. Create workout types slide - ONLY if workout data exists
        if "workout_total" in self.available_templates and has_workout_data and self.client_data.get('workout_types', []):
            workout_total_text = [
                {
            ...
            workout_total_slide = self.create_slide(
                "workout_total", workout_total_text)
            if workout_total_slide:
                slides.append(workout_total_slide)

        # 5. Create workout stats slide - ONLY if workout data exists
        if "workout_stats" in self.available_templates and has_workout_data and self.client_data.get('total_reps', 0) > 0:
            workout_stats_text = [
                {
            ...
            workout_stats_slide = self.create_slide(
                "workout_stats", workout_stats_text)
            if workout_stats_slide:
                slides.append(workout_stats_slide)

        # 6. Create workload increase slide - ONLY if workload data exists
        if "workload" in self.available_templates and has_workload_data:
            workload_text = [
                {
        ...
        if "exercise_level" in self.available_templates and has_workout_data:
            exercise_level_text = [
                {
            ...
            exercise_level_slide = self.create_slide(
                "exercise_level", exercise_level_text)
            if exercise_level_slide:
                slides.append(exercise_level_slide)

        # 8. Create strength gains slide - ONLY if exercise data exists
        if "strength_gains" in self.available_templates and self.client_data.get('data_availability', {}).get('exercise_data', has_workout_data):
            ...
            if len(top_exercises) < 3:
                ...
                strength_gains_slide = self.create_slide(
                    "strength_gains", strength_gains_text)
        ...
        if "nutrition" in self.available_templates and has_nutrition_data:
            nutrition_text = [
                {
        ...
        if "calories" in self.available_templates and has_nutrition_data:
            calories_text = [
                {
        ...
        if "weight_goal" in self.available_templates and has_weight_data:
            weight_goal_text = [
                {
                    "text": "Let's Check your",
                    "position": (None, center_y - 50),
            ...
            weight_goal_slide = self.create_slide(
                "weight_goal", weight_goal_text)
            if weight_goal_slide:
                slides.append(weight_goal_slide)

        # 12. Create current weight slide - ONLY if weight data exists
        if "current_weight" in self.available_templates and has_weight_data:
        ...
        if "progress_photos" in self.available_templates and self.client_data.get('data_availability', {}).get('up_to_date_photos', False):
            photos_text = [
                {
                    "text": "Progress",
                    "position": (None, center_y - 50),
            ...
            photos_slide = self.create_slide("progress_photos", photos_text)
            if photos_slide:
                slides.append(photos_slide)

        # 16. Create conclusion slide - personalized message (always include if template exists)
        if "conclusion" in self.available_templates:
            ...
            # Add a vertical offset to move text down on the screen
            vertical_offset = 100  # Pixels to move text down
            start_y = center_y - (len(message_lines) *
                                  line_height) // 2 + vertical_offset

            # Limit to 7 lines max
            for i, line in enumerate(message_lines[:7]):
                # Calculate proper position with enough margin from edges
        ...
        conclusion_slide = self.create_slide(
            "conclusion", conclusion_text, duration=5.0)
        if conclusion_slide:
            slides.append(conclusion_slide)

        # 17. Create motivational slide with bunny - "Keep Moving!" (always include if template exists)
        if "motivational" in self.available_templates:
            ...
            motivational_slide = self.create_slide(
                "motivational", motivational_text, duration=4.0)
        ...
        try:
            # First attempt with crossfadein transitions
            clips_with_transitions = [slides[0]]

            for i in range(1, len(slides)):
                try:
                    slides[i] = slides[i].set_start(
                        clips_with_transitions[-1].end - self.transition_duration)
                    slides[i] = slides[i].crossfadein(self.transition_duration)
                    clips_with_transitions.append(slides[i])
                except Exception as e:
                    print(f"Warning: Transition error on slide {i}: {e}")
                    # Fallback to simple append if transition fails
                    clips_with_transitions.append(slides[i])

            final_clip = CompositeVideoClip(clips_with_transitions)
        except Exception as e:
            print(f"Falling back to simple concatenation: {e}")
            # Fallback to simple concatenation if transitions fail
            final_clip = concatenate_videoclips(slides, method="compose")

        # Add background music if available
        # Look for music files in the music directory or the same directory as the script

        # Define possible music locations
        music_dir = os.path.join(self.base_dir, "music")
        music_files = []

        # Check if music directory exists and contains mp3 files
        ...
        # If no music files found in music directory, look in the current directory
        if not music_files:
            current_dir_music = [f for f in os.listdir(self.base_dir)
                                 if f.lower().endswith('.mp3')]
            music_files = [os.path.join(self.base_dir, f)
                           for f in current_dir_music]

        # Default to background_music.mp3 if it exists and no other music files found
        if not music_files and os.path.exists("background_music.mp3"):
            music_files = ["background_music.mp3"]

        if music_files:
            # Randomly select a music file
            selected_music = random.choice(music_files)
            print(f"Adding background music from: {selected_music}")
            try:
        ...
        final_clip.write_videofile(
            ...
            fps=30,
            codec="libx264",
            audio_codec="aac" if os.path.exists(self.music_path) else None,
            preset="medium"
        )
    ...
    def upload_to_google_drive(self):
        """
        Upload the generated video to Google Drive and return the shareable link.

        Returns:
            str: Shareable link to the video or None if upload fails.
        """
        try:
            # Check if the video file exists
            if not os.path.exists(self.output_path):
                logging.error(f"Video file not found at {self.output_path}")
                return None

            # Get or create the Fitness Wrapped Videos folder
            folder_id = google_drive_utils.get_or_create_fitness_wrapped_folder()

            # Upload the video to Google Drive
            success, file_id, web_view_link = google_drive_utils.upload_video_to_drive(
                str(self.output_path), folder_id
            )

            if success and web_view_link:
                logging.info(
                    f"Video uploaded to Google Drive: {web_view_link}")

                # Store the link in the client data
                self.client_data['google_drive_link'] = web_view_link

                # Save the updated client data
                client_name = self.client_data.get("name", "client")
                self.save_client_data_with_link(client_name, web_view_link)
            ...
            logging.exception(f"Error uploading video to Google Drive: {e}")
            return None

    def save_client_data_with_link(self, client_name, drive_link):
        """
        Save the client data with the Google Drive link.

        Args:
            client_name (str): Name of the client.
            drive_link (str): Google Drive link to the video.
        """
        try:
            ...
            # File path for the client data
            file_path = data_dir / f"{client_name.replace(' ', '_')}_data.json"

            # Add the Google Drive link to the client data
            client_data = self.client_data.copy()
            client_data['google_drive_link'] = drive_link
            client_data['upload_date'] = datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S")

            # Save the client data to a JSON file
            with open(file_path, 'w') as f:
                json.dump(client_data, f, indent=4)

            logging.info(
                f"Client data with Google Drive link saved to {file_path}")

        except Exception as e:
            logging.exception(f"Error saving client data with link: {e}")

    def generate_instagram_message(self):
        """
        Generate a message that can be sent on Instagram with the Google Drive link.

        Returns:
            str: Instagram message with the Google Drive link.
        """
        try:
            client_name = self.client_data.get("name", "Client")
            drive_link = self.client_data.get("google_drive_link", "")

            if not drive_link:
                return "No Google Drive link available for this video."

            message = f"Hi {client_name}! 🎉\n\n"
            message += "Your personalized Fitness Wrapped video is ready! 🏋️‍♀️💪\n\n"
            message += "Check out your progress and achievements from this week:\n"
            message += f"{drive_link}\n\n"
            message += "Let me know what you think! Keep up the great work! 💯"

            return message

        except Exception as e:
            logging.exception(f"Error generating Instagram message: {e}")
            return "Error generating Instagram message."
