    ...
    def analyze_image_with_gpt(self, image_path, known_username=None):
        ...
        try:
            ...
            if known_username:
                ...
                1. DESCRIPTION: Briefly describe what is happening in the image in 2-3 sentences. Be specific about visible content.
                ...
                2. COMMENT: Create a friendly, casual comment in Shannon's style (Male Australian Personal Trainer) to respond to this story:
                   ...
                   - Use casual Aussie PT slang (bro, babe, mate, lets go!, full send!)
                   - Focus on motivation and fitness encouragement
                   - If the person is male, use "bro"
                   - If the person is female, use "lovely" or "babe"
                ...
                For the comment specifically, consider the content type:
                - For food: "Yumo!" or "Food goals, bro/babe!"
                - For travel: "Stunning views, mate!" or "What an adventure, lovely!"
                - For gym/workout: "Killing it, bro/lovely!" or "Beast mode!"
                - For social/event: "Having a blast, bro/lovely!" or "Great company!"
                - For pets: "Such a good doggo, mate!" or "Cutie patootie!"
                - For relaxing content: "Living the dream, bro/mate!" or "Chilled vibes!"
                - For selfies: "Looking good!" or "You look amazing!"
                """
    ...
    def process_single_story(self, story_number, total_stories):
        ...
        try:
            ...
            if analysis_result:
                ...
                # Update Google Sheets with the Instagram lead info
                print_substep("Updating Instagram lead in Google Sheets...")
                sheet_update_success = self.update_instagram_lead(
                ...
                if sheet_update_success:
                    print("✔️ Successfully updated Instagram lead in Google Sheets")
                else:
                    print("⚠️ Failed to update Instagram lead in Google Sheets")
...
"""
    ...
    def setup_sheets_api(self):
        ...
        try:
            ...
            if not os.path.exists(SERVICE_ACCOUNT_FILE):
                ...
                SERVICE_ACCOUNT_FILE = os.path.join(
                    parent_dir, 'service-account-credentials.json')
    ...
    def update_instagram_lead(self, instagram_username, description, comment):
        """
        Updates the Instagram lead information in Google Sheets
        - Checks if username exists in column P
        - If username exists, only updates conversation in column AD
        - If username doesn't exist, adds a new row at the bottom with username and conversation
        """
        try:
            print(
                f"\nAttempting to update Instagram lead: {instagram_username}...")

            if not self.sheets_api:
                print("✗ Google Sheets API not initialized")
                if not self.setup_sheets_api():
                    return False

            # Your specific spreadsheet ID (same as in checkin_lastweek14.py)
            SPREADSHEET_ID = '1038Ep0lYGEtpipNAIzH7RB67-KOAfXA-TcUTKBKqIfo'

            # Step 1: Check if the Instagram user already exists in column P (case insensitive)
            try:
                print("Checking if user already exists in the spreadsheet...")
                ...
                if not rows:
                    ...
                    for i, row in enumerate(rows):
                        if row and row[0].strip().lower() == instagram_username.strip().lower():
                            row_index = i
                            print(
                                f"✓ EXISTING USER FOUND: {instagram_username} at row {row_index+1}")
                            break

                # Step 2: Either update existing user or create new one
                if row_index == -1:
                    # User doesn't exist - create new entry at the bottom of the list
                    print(
                        f"✓ NEW USER - Adding {instagram_username} as new entry")

                    # Get all data from column P to find the last filled row
                    ...
                    # Find the first empty row in column P (after all existing entries)
                    row_index = len(insta_rows)
                    print(
                        f"✓ Will add new Instagram user at row {row_index+1}")

                    # Update Instagram username in column P
                    range_name = f'Sheet1!P{row_index+1}'
                    ...
                    # For new entries, directly add the conversation text to column AD
                    conversation_text = f"Instagram Story - {description} - Commented: {comment}"
                    ad_range_name = f'Sheet1!AD{row_index+1}'

                    # Add new conversation to the cell (no need to check for existing text for new entries)
                    values = [[conversation_text]]
                    ...
                    # User exists - update only their conversation history in column AD
                    print(f"✓ UPDATING EXISTING USER: {instagram_username}")
                    conversation_text = f"Instagram Story - {description} - Commented: {comment}"
                    range_name = f'Sheet1!AD{row_index+1}'

                    # First get existing conversation
                    try:
                        existing = self.sheets_api.values().get(
                            spreadsheetId=SPREADSHEET_ID,
                            range=range_name
                        ).execute()

                        existing_text = ""
                        if 'values' in existing and existing['values'] and existing['values'][0]:
                            existing_text = existing['values'][0][0] + "\n\n"

                        # Update with new conversation added to existing
                        updated_text = existing_text + conversation_text
                        values = [[updated_text]]
                ...
                print(f"✗ Error finding Instagram user: {e}")
                return False

        except Exception as e:
            print(f"✗ Error in update_instagram_lead: {e}")
            return False
