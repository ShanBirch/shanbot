...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('smart_coach_simple_test.log'),
...
def test_nicole_lynch():
    ...
    try:
        ...
        print(
            f"\nNicole Lynch: {total_exercises} exercises across {len(smart_progressions)} workouts")

        for workout_name, exercises in smart_progressions.items():
            print(f"\n  📋 {workout_name}: {len(exercises)} exercises")
            for exercise in exercises:
                print(f"    • {exercise['exercise_name']}")
                print(f"      Current → Next: {exercise['reason']}")
                print(f"      Goals: {exercise['reps']}")

        # Save progressions for reference
        with open('nicole_lynch_smart_progressions.json', 'w') as f:
            json.dump({"Nicole Lynch": smart_progressions}, f, indent=2)

        print(f"\n💾 Progressions saved to nicole_lynch_smart_progressions.json")
        print(f"\n🎯 Ready to apply to Trainerize!")
        print(f"   Use this data with the weekly_program_updater_fixed.py system")

        # Show sample progression details
        if smart_progressions:
            sample_workout = list(smart_progressions.keys())[0]
            sample_exercise = smart_progressions[sample_workout][0]
            print(f"\n📊 Sample progression:")
            print(f"   Workout: {sample_workout}")
            print(f"   Exercise: {sample_exercise['exercise_name']}")
            print(f"   Progression: {sample_exercise['reason']}")
            print(f"   New Goals: {sample_exercise['reps']}")

    except Exception as e:
        logging.error(f"Error in smart coach simple test: {e}")
        print(f"❌ Error: {e}")
